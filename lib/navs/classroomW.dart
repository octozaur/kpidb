...
class ClassroomW extends StatefulWidget {
  var classroom;
  ClassroomW(this.classroom);
  @override
  _ClassroomW createState() => _ClassroomW(classroom);
}

class _ClassroomW extends State<ClassroomW> {
  var classroom;
  _ClassroomW(this.classroom);
  ...
  Future updateDB() async {
    if(starIndex != 0){
      ...
      var res = await DatabaseService().kpidbCollection.collection("classrooms").document(classroom).get();
      ...
      if(i>0)
        sum = sum / i;
      await DatabaseService().kpidbCollection.collection("classrooms").document(classroom).updateData({
        "ratings": dbl,
        "comments": str,
        "android_ids": ids,
        "current_rating": sum,
